(function() {
  ...
  angular
    .module('nonoApp')
    .controller('RecordsTransferDetailController', RecordsTransferDetailController);

  /** @ngInject */
  function RecordsTransferDetailController($rootScope, localStorageService, toastr, utils, $state, $stateParams, InvtApi, $timeout) {
    var vm = this;
    vm.obj = {
      reason:'',
      discount:'',
          ...
          onOk: function() {
            if (+vm.obj.type == 0) {
              InvtApi.upShelfForOneDebtSale({
                ...
                if (res.succeed) {
                  // var record = localStorageService.get('record');
                  // record.transferStatus = 2;
                  // record.transferMsg = "您已提交过转让，转让上架期间不能重复债转";
                  // localStorageService.set('record',record);
                  // var _type = $stateParams.type == 'debt'?3:2;
                  $state.go('mine');
